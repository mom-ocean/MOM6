...
subroutine build_sigma_column(CS, depth, totalThickness, zInterface)
  ...
  real,                     intent(in)    :: totalThickness !< Column thickness (positive in H ~> m or kg m-2)
  real, dimension(CS%nk+1), intent(inout) :: zInterface !< Absolute positions of interfaces in H ~> m or kg m-2
