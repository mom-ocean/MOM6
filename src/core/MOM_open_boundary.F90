     ...
     if (segment%direction == OBC_DIRECTION_E) then
       ...
       do k=1,nz ;  do j=segment%HI%jsd,segment%HI%jed
         if (segment%radiation) then
           ...
           segment%normal_vel(I,j,k) = US%m_s_to_L_T*(u_new(I,j,k) + rx_avg*u_new(I-1,j,k)) / (1.0+rx_avg)
         ...
         elseif (segment%oblique) then
           ...
           segment%normal_vel(I,j,k) = US%m_s_to_L_T*((cff_avg*u_new(I,j,k) + rx_avg*u_new(I-1,j,k)) - &
         ...
         elseif (segment%gradient) then
           segment%normal_vel(I,j,k) = US%m_s_to_L_T*u_new(I-1,j,k)
         endif
       ...
       if (segment%radiation_tan .or. segment%radiation_grad) then
         ...
         if (segment%radiation_tan) then
           do k=1,nz ;  do J=segment%HI%JsdB,segment%HI%JedB
             ...
             segment%tangential_vel(I,J,k) = US%m_s_to_L_T*(v_new(I,J,k) + rx_avg*v_new(I-1,J,k)) / (1.0+rx_avg)
           enddo ; enddo
         endif
...
!              rx_avg = 0.0
!            endif
             segment%tangential_grad(I,J,k) = ((v_new(i,J,k) - v_new(i-1,J,k))*G%US%m_to_L*G%IdxBu(I-1,J) + &
                               rx_avg*(v_new(i-1,J,k) - v_new(i-2,J,k))*G%US%m_to_L*G%IdxBu(I-2,J)) / (1.0+rx_avg)
           enddo ; enddo
         endif
       ...
       if (segment%oblique_tan .or. segment%oblique_grad) then
         ...
         if (segment%oblique_tan) then
           do k=1,nz ;  do J=segment%HI%JsdB+1,segment%HI%JedB-1
             ...
             segment%tangential_vel(I,J,k) = US%m_s_to_L_T*((cff_avg*v_new(i,J,k) + rx_avg*v_new(i-1,J,k)) - &
              (max(ry_avg,0.0)*segment%grad_tan(j,2,k) + min(ry_avg,0.0)*segment%grad_tan(j+1,2,k))) / &
                            (cff_avg + rx_avg)
           enddo ; enddo
         endif
         if (segment%nudged_tan) then
         ...
         if (segment%oblique_grad) then
           ...
           do k=1,nz ;  do J=segment%HI%JsdB+1,segment%HI%JedB-1
             ...
             ry_avg = ry_tangential(I,J,k)
             cff_avg = cff_tangential(I,J,k)
             segment%tangential_grad(I,J,k) = ((cff_avg*(v_new(i,J,k)  - v_new(i-1,J,k))*G%US%m_to_L*G%IdxBu(I-1,J) &
                + rx_avg*(v_new(i-1,J,k) - v_new(i-2,J,k))*G%US%m_to_L*G%IdxBu(I-2,J)) - &
              (max(ry_avg,0.0)*segment%grad_gradient(J,2,k) + min(ry_avg,0.0)*segment%grad_gradient(J+1,2,k))) / &
                            (cff_avg + rx_avg)
           enddo ; enddo
         endif
         if (segment%nudged_grad) then
     ...
     if (segment%direction == OBC_DIRECTION_W) then
       ...
       if (segment%radiation_tan .or. segment%radiation_grad) then
         ...
         if (segment%radiation_tan) then
           do k=1,nz ;  do J=segment%HI%JsdB,segment%HI%JedB
             rx_avg = rx_tangential(I,J,k)
             segment%tangential_vel(I,J,k) = US%m_s_to_L_T*(v_new(i+1,J,k) + rx_avg*v_new(i+2,J,k)) / (1.0+rx_avg)
           enddo ; enddo
         endif
         if (segment%nudged_tan) then
           do k=1,nz ; do J=segment%HI%JsdB,segment%HI%JedB
...
!              rx_avg = 0.0
!            endif
             segment%tangential_grad(I,J,k) = ((v_new(i+2,J,k) - v_new(i+1,J,k))*G%US%m_to_L*G%IdxBu(I+1,J) + &
                               rx_avg*(v_new(i+3,J,k) - v_new(i+2,J,k))*G%US%m_to_L*G%IdxBu(I+2,J)) / (1.0+rx_avg)
           enddo ; enddo
         endif
         if (segment%nudged_grad) then
           do k=1,nz ; do J=segment%HI%JsdB,segment%HI%JedB
       ...
       if (segment%oblique_tan .or. segment%oblique_grad) then
         ...
         if (segment%oblique_tan) then
           do k=1,nz ;  do J=segment%HI%JsdB+1,segment%HI%JedB-1
             ...
             ry_avg = ry_tangential(I,J,k)
             cff_avg = cff_tangential(I,J,k)
             segment%tangential_vel(I,J,k) = US%m_s_to_L_T*((cff_avg*v_new(i+1,J,k) + rx_avg*v_new(i+2,J,k)) - &
              (max(ry_avg,0.0)*segment%grad_tan(j,2,k) + min(ry_avg,0.0)*segment%grad_tan(j+1,2,k))) / &
                            (cff_avg + rx_avg)
           enddo ; enddo
         endif
         ...
         if (segment%oblique_grad) then
           ...
           do k=1,nz ;  do J=segment%HI%JsdB+1,segment%HI%JedB-1
             ...
             ry_avg = ry_tangential(I,J,k)
             cff_avg = cff_tangential(I,J,k)
             segment%tangential_grad(I,J,k) = ((cff_avg*(v_new(i+2,J,k)  - v_new(i+1,J,k))*G%US%m_to_L*G%IdxBu(I+1,J) &
                + rx_avg*(v_new(i+3,J,k) - v_new(i+2,J,k))*G%US%m_to_L*G%IdxBu(I+2,J)) - &
              (max(ry_avg,0.0)*segment%grad_gradient(J,2,k) + min(ry_avg,0.0)*segment%grad_gradient(J+1,2,k))) / &
                            (cff_avg + rx_avg)
           enddo ; enddo
         endif
     ...
     if (segment%direction == OBC_DIRECTION_N) then
       ...
       do k=1,nz ;  do i=segment%HI%isd,segment%HI%ied
         if (segment%radiation) then
           ...
           segment%normal_vel(i,J,k) = US%m_s_to_L_T*(v_new(i,J,k) + ry_avg*v_new(i,J-1,k)) / (1.0+ry_avg)
           ! Copy restart fields into 3-d arrays. This is an inefficient and temporary issues
           ! implemented as a work-around to limitations in restart capability
           OBC%ry_normal(i,J,k) = segment%ry_normal(i,J,k)
         elseif (segment%oblique) then
           ...
           OBC%cff_normal(i,J,k) = segment%cff_normal(i,J,k)
         elseif (segment%gradient) then
           segment%normal_vel(i,J,k) = US%m_s_to_L_T*v_new(i,J-1,k)
         endif
         if ((segment%radiation .or. segment%oblique) .and. segment%nudged) then
           ! dhdt gets set to 0 on inflow in oblique case
       ...
       if (segment%radiation_tan .or. segment%radiation_grad) then
         ...
         if (segment%radiation_tan) then
           do k=1,nz ;  do I=segment%HI%IsdB,segment%HI%IedB
             rx_avg = rx_tangential(I,J,k)
             segment%tangential_vel(I,J,k) = US%m_s_to_L_T*(u_new(I,j,k) + rx_avg*u_new(I,j-1,k)) / (1.0+rx_avg)
           enddo ; enddo
         endif
         if (segment%nudged_tan) then
           do k=1,nz ; do I=segment%HI%IsdB,segment%HI%IedB
...
!              rx_avg = 0.0
!            endif
             segment%tangential_grad(I,J,k) = ((u_new(I,j,k) - u_new(I,j-1,k))*G%US%m_to_L*G%IdyBu(I,J-1) + &
                               rx_avg*(u_new(I,j-1,k) - u_new(I,j-2,k))*G%US%m_to_L*G%IdyBu(I,J-2)) / (1.0+rx_avg)
           enddo ; enddo
         endif
         if (segment%nudged_grad) then
           do k=1,nz ; do I=segment%HI%IsdB,segment%HI%IedB
       ...
       if (segment%oblique_tan .or. segment%oblique_grad) then
         ...
         if (segment%oblique_grad) then
           ...
           do k=1,nz ;  do I=segment%HI%IsdB+1,segment%HI%IedB-1
             ...
             ry_avg = ry_tangential(I,J,k)
             cff_avg = cff_tangential(I,J,k)
             segment%tangential_grad(I,J,k) = ((cff_avg*(u_new(I,j,k)  - u_new(I,j-1,k))*G%US%m_to_L*G%IdyBu(I,J-1) &
                + rx_avg*(u_new(I,j-1,k) - u_new(I,j-2,k))*G%US%m_to_L*G%IdyBu(I,J-2)) - &
              (max(ry_avg,0.0)*segment%grad_gradient(I,2,k) + min(ry_avg,0.0)*segment%grad_gradient(I+1,2,k))) / &
                            (cff_avg + rx_avg)
           enddo ; enddo
         endif
         if (segment%nudged_grad) then
     ...
     if (segment%direction == OBC_DIRECTION_S) then
       ...
       if (segment%radiation_tan .or. segment%radiation_grad) then
         ...
         if (segment%radiation_tan) then
           do k=1,nz ;  do I=segment%HI%IsdB,segment%HI%IedB
             rx_avg = rx_tangential(I,J,k)
             segment%tangential_vel(I,J,k) = US%m_s_to_L_T*(u_new(I,j+1,k) + rx_avg*u_new(I,j+2,k)) / (1.0+rx_avg)
           enddo ; enddo
         endif
         if (segment%nudged_tan) then
           do k=1,nz ; do I=segment%HI%IsdB,segment%HI%IedB
...
!              rx_avg = 0.0
!            endif
             segment%tangential_grad(I,J,k) = ((u_new(I,j+2,k) - u_new(I,j+1,k))*G%US%m_to_L*G%IdyBu(I,J+1) + &
                               rx_avg*(u_new(I,j+3,k) - u_new(I,j+2,k))*G%US%m_to_L*G%IdyBu(I,J+2)) / (1.0+rx_avg)
           enddo ; enddo
         endif
         if (segment%nudged_grad) then
           do k=1,nz ; do J=segment%HI%JsdB,segment%HI%JedB
       ...
       if (segment%oblique_tan .or. segment%oblique_grad) then
         ...
         if (segment%oblique_grad) then
           ...
           do k=1,nz ;  do I=segment%HI%IsdB+1,segment%HI%IedB-1
             ...
             ry_avg = ry_tangential(I,J,k)
             cff_avg = cff_tangential(I,J,k)
             segment%tangential_grad(I,J,k) = ((cff_avg*(u_new(I,j+2,k)  - u_new(I,j+1,k))*G%US%m_to_L*G%IdyBu(I,J+1) &
                + rx_avg*(u_new(I,j+3,k) - u_new(I,j+2,k))*G%US%m_to_L*G%IdyBu(I,J+2)) - &
              (max(ry_avg,0.0)*segment%grad_gradient(i,2,k) + min(ry_avg,0.0)*segment%grad_gradient(i+1,2,k))) / &
                            (cff_avg + rx_avg)
           enddo ; enddo
         endif
    ...
    elseif (segment%radiation .or. segment%oblique .or. segment%gradient) then
      if (segment%is_E_or_W) then
        ...
        do k=1,G%ke ;  do j=segment%HI%jsd,segment%HI%jed
          u(I,j,k) = G%US%L_T_to_m_s*segment%normal_vel(I,j,k)
        enddo ; enddo
      elseif (segment%is_N_or_S) then
        J=segment%HI%JsdB
        do k=1,G%ke ;  do i=segment%HI%isd,segment%HI%ied
          v(i,J,k) = G%US%L_T_to_m_s*segment%normal_vel(i,J,k)
        enddo ; enddo
      endif
    endif
  enddo
