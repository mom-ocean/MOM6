...
!$OMP                                  u_bc_accel,vp,v,v_bc_accel)
  ...
  if (CS%debug) then
    call uvchksum_pair(up,"before vertvisc: up", &
                       vp,"before vertvisc: vp",G%HI,haloshift=0)
  endif
...
!$OMP                                  dt_pred,v_bc_accel,CS,up,u_init,u_bc_accel)
  ...
  if (CS%debug) then
    call uvchksum_pair(up,"Predictor 1 u", vp,"Predictor 1 v", G%HI, haloshift=0)
...
! u_av  <- u_av  + dt_pred d/dz visc d/dz u_av
  call cpu_clock_begin(id_clock_vertvisc)
  if (CS%debug) then
    call uvchksum_pair(up,"0 before vertvisc: up", &
                       vp,"0 before vertvisc: vp",G%HI,haloshift=0)
  endif
