...
type, public :: forcing
  ...
  netMassOut    => NULL(), & !< Sum of water mass flux out of the ocean ( kg/(m^2 s) )
  netMassinOut  => NULL()    !< Net water mass flux into of the ocean ( kg/(m^2 s) )
...
subroutine extractFluxes1d(G, GV, fluxes, optics, nsw, j, dt,                           &
  ...
  !   if(ASSOCIATED(fluxes%heat_content_cond)) fluxes%heat_content_cond(i,j) = 0.0 !??? --AJA
    ...
    ! Store these calculations in the fluxes structure
    fluxes%netMassInOut(i,j) = netMassInOut(i)
...
subroutine allocate_forcing_type(G, fluxes, stress, ustar, water, heat, shelf, press)
  ...
  call myAlloc(fluxes%netMassInOut,isd,ied,jsd,jed, water)
