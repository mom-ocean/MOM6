...
subroutine register_forcing_type_diags(Time, diag, use_temperature, handles)
 ...
 handles%id_fprec_ga = register_scalar_field('ocean_model', 'fprec_ga', Time, diag,&
  ...
  handles%id_net_heat_surface = register_diag_field('ocean_model', 'net_heat_surface',diag%axesT1,  &
        Time,'Surface ocean heat flux from SW+LW+lat+sens+mass transfer+frazil+added', 'Watt/m^2',&
  ...
  handles%id_heat_added = register_diag_field('ocean_model', 'heat_added', diag%axesT1, Time, &
        'Ficticious surface heat flux into ocean', 'Watt/m^2')
  ...
  handles%id_total_net_heat_surface = register_scalar_field('ocean_model',                      &
      ...
      long_name='Area integrated surface heat flux from SW+LW+lat+sens+mass+frazil+added',    &
      units='Watt',                                                                             &
  ...
  handles%id_total_heat_added = register_scalar_field('ocean_model',&
      'total_heat_added', Time, diag,                               &
      long_name='Area integrated surface heat flux from ficticious sources',   &
      units='Watt')
  ...
  handles%id_net_heat_surface_ga = register_scalar_field('ocean_model',                       &
      ...
      long_name='Area averaged surface heat flux from SW+LW+lat+sens+mass+frazil+added',    &
      units='W m-2',                                                                          &
  ...
  handles%id_saltFluxAdded = register_diag_field('ocean_model', 'salt_flux_added', &
        diag%axesT1,Time,'Salt flux into ocean at surface due to ficticous sources',           &
        'kilogram/(meter^2 * second)')
  ...
  handles%id_total_saltFluxAdded = register_scalar_field('ocean_model', 'total_salt_Flux_Added', &
      Time, diag, long_name='Area integrated surface salt flux due to ficticous sources', units='kg')
...
!! \f[
!!  \mbox{hfds = shortwave + longwave + latent + sensible + mass transfer + frazil + added}
!! \f]
!!
...
!!  * added (RES)     = heat from ficticious sources including surface damping
