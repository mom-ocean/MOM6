...
subroutine step_MOM(fluxes, state, Time_start, time_interval, CS)
  ...
  do n=1,n_max
    ...
    if (CS%diabatic_first .and. (CS%dt_trans==0.0)) then ! do thermodynamics.
      ...
      if (CS%debug) then
        call uvchksum_pair("Pre set_viscous_BBL [uv]", u, v, G%HI, haloshift=1)
...
!         call pass_vector(u, v, G%Domain)
        ...
        if (CS%debug) then
          call uvchksum_pair("Post-dia first [uv]", u, v, G%HI, haloshift=2)
          call hchksum(h,"Post-dia first h", G%HI, haloshift=1)
          call uvchksum_pair("Post-dia first [uv]h", &
    ...
    if (CS%mixedlayer_restrat) then
      if (CS%debug) then
        ...
        call uvchksum_pair("Pre-mixedlayer_restrat uhtr", &
                           CS%uhtr, CS%vhtr, G%HI, haloshift=0)
      endif
      ...
      if (CS%debug) then
        ...
        call uvchksum_pair("Post-mixedlayer_restrat [uv]htr", &
                           CS%uhtr, CS%vhtr, G%HI, haloshift=0)
      endif
    endif
    ...
    if (do_advection) then
      ...
      if (CS%debug) then
        call uvchksum_pair("Pre-advection [uv]", u, v, G%HI, haloshift=2)
        call hchksum(h*GV%H_to_m,"Pre-advection h", G%HI, haloshift=1)
        call uvchksum_pair("Pre-advection uhtr", CS%uhtr*GV%H_to_m, &
      ...
      if (.not.CS%diabatic_first) then ; if (.not.CS%adiabatic) then
        ...
        if (CS%debug) then
          call uvchksum_pair("Pre-diabatic [uv]", u, v, G%HI, haloshift=2)
          call hchksum(h*GV%H_to_m,"Pre-diabatic h", G%HI, haloshift=1)
          call uvchksum_pair("Pre-diabatic [uv]h", CS%uhtr*GV%H_to_m, &
                             CS%vhtr*GV%H_to_m, G%HI, haloshift=0)
...
!         call pass_vector(u, v, G%Domain)
        ...
        if (CS%debug) then
          call uvchksum_pair("Post-diabatic u", u, v, G%HI, haloshift=2)
          call hchksum(h*GV%H_to_m,"Post-diabatic h", G%HI, haloshift=1)
          call uvchksum_pair("Post-diabatic [uv]h", CS%uhtr*GV%H_to_m, &
                             CS%vhtr*GV%H_to_m, G%HI, haloshift=0)
...
!  dG%g_Earth = GV%g_Earth
  ...
  if (ALE_remap_init_conds(CS%ALE_CSp) .and. .not. query_initialized(CS%h,"h",CS%restart_CSp)) then
    ...
    ! all examples.
    if (CS%debug) then
      call uvchksum_pair("Pre ALE adjust init cond [uv]", &
    ...
    if (CS%debug) then
      call uvchksum_pair("Post ALE adjust init cond [uv]", &
                         CS%u, CS%v, G%HI, haloshift=1)
