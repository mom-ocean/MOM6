...
use MOM_domains, only : compute_extent
...
type :: block_type
  ...
  !--- The following are used in PressureForce_AFV_nonBouss
  real, dimension(:,:), allocatable :: &
    ...
    inty_za    ! The meridional integral of the geopotential anomaly along the
               ! interface below a layer, divided by the grid spacing, m2 s-2.
  !--- The following are used in PressureForce_AFV_Bouss
  real, dimension(:,:), allocatable :: &
...
type(ocean_grid_type), allocatable :: G_BLK(:) 
...
!$OMP parallel do default(none) shared(nblocks,block,nz,za,G,dza,intx_dza,h,PFu, &
!$OMP                                  intp_dza,p,dp_neglect,inty_dza,PFv,CS,dM) &
!$OMP                          private(is,ie,js,je,Isq,Ieq,Jsq,Jeq)
  do n = 1, nblocks
    is=block(n)%isc; ie=block(n)%iec; js=block(n)%jsc; je=block(n)%jec
    Isq=block(n)%IscB; Ieq=block(n)%IecB; Jsq=block(n)%JscB; Jeq=block(n)%JecB
    do j=Jsq,Jeq+1 ; do i=Isq,Ieq+1
      block(n)%za(i,j) = za(i,j)
    enddo ; enddo
    do j=js,je ; do I=Isq,Ieq
      block(n)%intx_za(I,j) = 0.5*(za(i,j) + za(i+1,j))
    enddo ; enddo
    do J=Jsq,Jeq ; do i=is,ie
      block(n)%inty_za(i,J) = 0.5*(za(i,j) + za(i,j+1))
    enddo ; enddo
    do k=1,nz
      ! These expressions for the acceleration have been carefully checked in
      ! a set of idealized cases, and should be bug-free.
      do j=Jsq,Jeq+1 ; do i=Isq,Ieq+1
        block(n)%dp(i,j) = G%H_to_Pa*h(i,j,k)
        block(n)%za(i,j) = block(n)%za(i,j) - dza(i,j,k)
      enddo ; enddo
      do j=js,je ; do I=Isq,Ieq
        block(n)%intx_za(I,j) = block(n)%intx_za(I,j) - intx_dza(I,j,k)
        PFu(I,j,k) = (((block(n)%za(i,j)*block(n)%dp(i,j) + intp_dza(i,j,k)) - &
                     (block(n)%za(i+1,j)*block(n)%dp(i+1,j) + intp_dza(i+1,j,k))) + &
                     ((block(n)%dp(i+1,j) - block(n)%dp(i,j)) * block(n)%intx_za(I,j) - &
                     (p(i+1,j,K) - p(i,j,K)) * intx_dza(I,j,k))) * &
                     (2.0*G%IdxCu(I,j) / ((block(n)%dp(i,j) + block(n)%dp(i+1,j)) + &
                     dp_neglect))
      enddo ; enddo
      do J=Jsq,Jeq ; do i=is,ie
        block(n)%inty_za(i,J) = block(n)%inty_za(i,J) - inty_dza(i,J,k)
        PFv(i,J,k) = (((block(n)%za(i,j)*block(n)%dp(i,j) + intp_dza(i,j,k)) - &
                     (block(n)%za(i,j+1)*block(n)%dp(i,j+1) + intp_dza(i,j+1,k))) + &
                     ((block(n)%dp(i,j+1) - block(n)%dp(i,j)) * block(n)%inty_za(i,J) - &
                     (p(i,j+1,K) - p(i,j,K)) * inty_dza(i,J,k))) * &
                     (2.0*G%IdyCv(i,J) / ((block(n)%dp(i,j) + block(n)%dp(i,j+1)) + &
                     dp_neglect))
      enddo ; enddo
...
!                  with any tidal contributions or compressibility compensation.
  ...
  real, parameter :: C1_6 = 1.0/6.0
  integer :: is, ie, js, je, Isq, Ieq, Jsq, Jeq, nz, nkmb
  integer :: i, j, k, n, isd, ied, jsd, jed
  integer :: PRScheme
...
!$OMP parallel do default(none) shared(nblocks,block,G_BLK,use_p_atm,rho_ref,G,e,     &
!$OMP                                  p_atm,nz,use_EOS,use_ALE,PRScheme,T_t,T_b,S_t, &
!$OMP                                  S_b,CS,tv,tv_tmp,h,PFu,I_Rho0,h_neglect,PFv,dM)&
!$OMP                          private(is,ie,js,je,Isq,Ieq,Jsq,Jeq,isd,ied,jsd,jed)
do n = 1, nblocks
  is=block(n)%isc; ie=block(n)%iec; js=block(n)%jsc; je=block(n)%jec
  Isq=block(n)%IscB; Ieq=block(n)%IecB; Jsq=block(n)%JscB; Jeq=block(n)%JecB
  isd=block(n)%isd; ied=block(n)%ied; jsd=block(n)%jsd; jed=block(n)%jed
  ...
  if (use_p_atm) then
    do j=Jsq,Jeq+1 ; do i=Isq,Ieq+1
      block(n)%pa(i,j) = (rho_ref*G%g_Earth)*e(i,j,1) + p_atm(i,j)
    enddo ; enddo
  else
    do j=Jsq,Jeq+1 ; do i=Isq,Ieq+1
      block(n)%pa(i,j) = (rho_ref*G%g_Earth)*e(i,j,1)
    enddo ; enddo
  endif
  do j=js,je ; do I=Isq,Ieq
    block(n)%intx_pa(I,j) = 0.5*(block(n)%pa(i,j) + block(n)%pa(i+1,j))
  enddo ; enddo
  do J=Jsq,Jeq ; do i=is,ie
    block(n)%inty_pa(i,J) = 0.5*(block(n)%pa(i,j) + block(n)%pa(i,j+1))
  enddo ; enddo
       ...
       if ( use_ALE ) then
          if ( PRScheme == PRESSURE_RECONSTRUCTION_PLM ) then
             call int_density_dz_generic_plm ( T_t(isd:ied,jsd:jed,k), &
                  ...
                  G_BLK(n), tv%eqn_of_state, Block(n)%dpa, Block(n)%intz_dpa, &
                  Block(n)%intx_dpa, Block(n)%inty_dpa, &
          ...
          elseif ( PRScheme == PRESSURE_RECONSTRUCTION_PPM ) then
             call int_density_dz_generic_ppm ( tv%T(isd:ied,jsd:jed,k), &
                  ...
                  S_b(isd:ied,jsd:jed,k), e(isd:ied,jsd:jed,K), &
                  e(isd:ied,jsd:jed,K+1), rho_ref, CS%Rho0, G%G_Earth, &
                  G, tv%eqn_of_state, Block(n)%dpa, Block(n)%intz_dpa, &
                  Block(n)%intx_dpa, Block(n)%inty_dpa)
          endif
       else
          call int_density_dz(tv_tmp%T(isd:ied,jsd:jed,k), tv_tmp%S(isd:ied,jsd:jed,k), &
               ...
               Block(n)%dpa, Block(n)%intz_dpa, Block(n)%intx_dpa, Block(n)%inty_dpa)
       endif
    else
      do j=Jsq,Jeq+1 ; do i=Isq,Ieq+1
        Block(n)%dz(i,j) = G%g_Earth*G%H_to_m*h(i,j,k)
        Block(n)%dpa(i,j) = (G%Rlay(k) - rho_ref)*Block(n)%dz(i,j)
        Block(n)%intz_dpa(i,j) = 0.5*(G%Rlay(k) - rho_ref)*Block(n)%dz(i,j)*h(i,j,k)
      enddo ; enddo
      do j=js,je ; do I=Isq,Ieq
        Block(n)%intx_dpa(I,j) = 0.5*(G%Rlay(k) - rho_ref) * (Block(n)%dz(i,j)+Block(n)%dz(i+1,j))
      enddo ; enddo
      do J=Jsq,Jeq ; do i=is,ie
        Block(n)%inty_dpa(i,J) = 0.5*(G%Rlay(k) - rho_ref) * (Block(n)%dz(i,j)+Block(n)%dz(i,j+1))
      enddo ; enddo
    endif

    ! Compute pressure gradient in x direction
    do j=js,je ; do I=Isq,Ieq
      PFu(I,j,k) = (((Block(n)%pa(i,j)*h(i,j,k) + Block(n)%intz_dpa(i,j)) - &
                     (Block(n)%pa(i+1,j)*h(i+1,j,k) + Block(n)%intz_dpa(i+1,j))) + &
                    ((h(i+1,j,k) - h(i,j,k)) * Block(n)%intx_pa(I,j) - &
                     (e(i+1,j,K+1) - e(i,j,K+1)) * Block(n)%intx_dpa(I,j))) * &
                   ((2.0*I_Rho0*G%IdxCu(I,j)) / &
                    ((h(i,j,k) + h(i+1,j,k)) + h_neglect))
      Block(n)%intx_pa(I,j) = Block(n)%intx_pa(I,j) + Block(n)%intx_dpa(I,j)
    enddo ; enddo
    ! Compute pressure gradient in y direction
    do J=Jsq,Jeq ; do i=is,ie
      PFv(i,J,k) = (((Block(n)%pa(i,j)*h(i,j,k) + Block(n)%intz_dpa(i,j)) - &
                     (Block(n)%pa(i,j+1)*h(i,j+1,k) + Block(n)%intz_dpa(i,j+1))) + &
                    ((h(i,j+1,k) - h(i,j,k)) * Block(n)%inty_pa(i,J) - &
                     (e(i,j+1,K+1) - e(i,j,K+1)) * Block(n)%inty_dpa(i,J))) * &
                   ((2.0*I_Rho0*G%IdyCv(i,J)) / &
                    ((h(i,j,k) + h(i,j+1,k)) + h_neglect))
      Block(n)%inty_pa(i,J) = Block(n)%inty_pa(i,J) + Block(n)%inty_dpa(i,J)
    enddo ; enddo
    do j=Jsq,Jeq+1 ; do i=Isq,Ieq+1
      Block(n)%pa(i,j) = Block(n)%pa(i,j) + Block(n)%dpa(i,j)
    enddo ; enddo
  enddo
...
#include "version_variable.h"
  character(len=40)  :: modname  ! This module's name.
  integer, allocatable :: ibegin(:), iend(:), jbegin(:), jend(:)
  integer              :: isd, ied, jsd, jed, IsdB, IedB, JsdB, JedB
  integer              :: isc, iec, jsc, jec, IscB, IecB, JscB, JecB
  integer              :: n, halo, i, j
  integer :: xblock = 1 !number of blocks in x-direction on each processor(for openmp)
  integer :: yblock = 1 !number of blocks in y-direction on each processor(for openmp)
  ...
  modname = "MOM_PressureForce_AFV"
  call log_version(param_file, modname, version)
  call get_param(param_file, modname, "RHO_0", CS%Rho0, &
                 "The mean ocean density used with BOUSSINESQ true to \n"//&
                 "calculate accelerations and the mass for conservation \n"//&
                 "properties, or with BOUSSINSEQ false to convert some \n"//&
                 "parameters from vertical units of m to kg m-2.", &
                 units="kg m-3", default=1035.0)
  call get_param(param_file, modname, "TIDES", CS%tides, &
                 "If true, apply tidal momentum forcing.", default=.false.)
  call get_param(param_file, modname, "MASS_WEIGHT_IN_PRESSURE_GRADIENT", CS%useMassWghtInterp, &
  ...
  call log_param(param_file, modname, "GFS / G_EARTH", CS%GFS_scale)

  call get_param(param_file, modname, "NI_BLOCK", xblock, "The number of blocks "// &
                 "in the x-direction on each processor(for openmp).", default=1)
  call get_param(param_file, modname, "NJ_BLOCK", yblock, "The number of blocks "// &
                 "in the y-direction on each processor(for openmp).", default=1)

  halo = G%isc - G%isd
  nblocks = xblock * yblock
  allocate(ibegin(xblock), iend(xblock), jbegin(yblock), jend(yblock))
  allocate(block(nblocks))
  if (G%Boussinesq) allocate(G_BLK(nblocks))
  call compute_extent(G%isc,G%iec,xblock,ibegin,iend)
  call compute_extent(G%jsc,G%jec,yblock,jbegin,jend)
  do n = 1,nblocks
    i = mod((n-1), xblock) + 1
    j = (n-1)/xblock + 1
    isc = ibegin(i) ; iec = iend(i) ; jsc = jbegin(j) ; jec = jend(j)
    IscB = isc ; IecB = iec ; JscB = jsc ; JecB = jec
    ...
    IsdB = IscB - halo ; IedB = IecB + halo
    JsdB = JscB - halo ; JedB = JecB + halo
    block(n)%isc = isc ; block(n)%iec = iec
    block(n)%jsc = jsc ; block(n)%jec = jec
    block(n)%isd = isd ; block(n)%ied = ied
    block(n)%jsd = jsd ; block(n)%jed = jed
    block(n)%IscB = IscB ; block(n)%IecB = IecB
    block(n)%JscB = JscB ; block(n)%JecB = JecB
    if (G%Boussinesq) then
      G_BLK(n)%H_subroundoff = G%H_subroundoff
      G_BLK(n)%isc = isc-isd+1 ; G_BLK(n)%iec = iec-isd+1
      G_BLK(n)%jsc = jsc-jsd+1 ; G_BLK(n)%jec = jec-jsd+1
      G_BLK(n)%IscB = IscB-isd+1 ; G_BLK(n)%IecB = IecB-isd+1
      G_BLK(n)%JscB = JscB-jsd+1 ; G_BLK(n)%JecB = JecB-jsd+1
      G_BLK(n)%isd = G_BLK(n)%isc - halo
      G_BLK(n)%ied = G_BLK(n)%iec + halo
      G_BLK(n)%jsd = G_BLK(n)%jsc - halo
      G_BLK(n)%jed = G_BLK(n)%jec + halo
      G_BLK(n)%IsdB = G_BLK(n)%IscB - halo
      G_BLK(n)%IedB = G_BLK(n)%Iecb + halo
      G_BLK(n)%Jsdb = G_BLK(n)%JscB - halo
      G_BLK(n)%Jedb = G_BLK(n)%JecB + halo
      allocate(G_BLK(n)%bathyT(G_BLK(n)%isd:G_BLK(n)%ied,G_BLK(n)%jsd:G_BLK(n)%jed))
      G_BLK(n)%bathyT(:,:) = G%bathyT(isd:ied,jsd:jed)

      allocate(block(n)%dz      (isd :ied ,jsd :jed ))
      allocate(block(n)%pa      (isd :ied ,jsd :jed ))
      allocate(block(n)%intx_pa (IsdB:IedB,jsd :jed ))
      allocate(block(n)%inty_pa (isd :ied ,JsdB:JedB))
      allocate(block(n)%dpa     (isd :ied ,jsd :jed ))
      allocate(block(n)%intx_dpa(IsdB:IedB,jsd :jed ))
      allocate(block(n)%inty_dpa(isd :ied ,JsdB:JedB))
      allocate(block(n)%intz_dpa(isd :ied ,jsd :jed ))
      block(n)%dz = 0 ; block(n)%pa = 0 
      block(n)%intx_pa = 0 ; block(n)%inty_pa = 0
      block(n)%dpa = 0 ; block(n)%intx_dpa = 0
      block(n)%inty_dpa = 0 ; block(n)%intz_dpa = 0
    else
      allocate(block(n)%dp     (isd :ied ,jsd :jed ))
      allocate(block(n)%za     (isd :ied ,jsd :jed ))
      allocate(block(n)%intx_za(IsdB:IedB,jsd :jed ))
      allocate(block(n)%inty_za(isd :ied ,JsdB:JedB))
      block(n)%dp = 0 ; block(n)%za = 0
      block(n)%intx_za = 0 ; block(n)%inty_za = 0
    endif
  enddo
