...
!GOMP(parallel do default(shared) private(i, j, k))
    do k=1,nz ; do j=LB%jsh,LB%jeh ; do i=LB%ish,LB%ieh
      h(i,j,k) = hin(i,j,k) - dt*G%IDXDYh(i,j) * (vh(i,J,k) - vh(i,J-1,k))
    enddo ; enddo ; enddo
    call cpu_clock_end(id_clock_update)
...
!GOMP(parallel do default(shared) private(i, j, k))
    do k=1,nz ; do j=LB%jsh,LB%jeh ; do i=LB%ish,LB%ieh
      h(i,j,k) = h(i,j,k) - dt* G%IDXDYh(i,j) * (uh(I,j,k) - uh(I-1,j,k))
  !   This line prevents underflow.
      if (h(i,j,k) < G%Angstrom) h(i,j,k) = G%Angstrom
    enddo ; enddo ; enddo
    call cpu_clock_end(id_clock_update)
...
!                       open face areas as a function of barotropic flow.
  ...
  logical, dimension(SZIQ_(G)) :: do_i
  real, dimension(SZIQ_(G),SZK_(G)) :: &
  ...
  do j=jsh,jeh
    ...
    if (present(uhbt) .or. do_aux .or. set_BT_cont) then
      ...
      do I=ish-1,ieh
        ...
        if (CS%vol_CFL) then
          dx_W = ratio_max(G%DXDYh(i,j), G%dy_u(I,j), 1000.0*G%DXh(i,j))
          dx_E = ratio_max(G%DXDYh(i+1,j), G%dy_u(I,j), 1000.0*G%DXh(i+1,j))
        else ; dx_W = G%DXh(i,j) ; dx_E = G%DXh(i+1,j) ; endif
        ...
        if (any_simple_OBC) then
          do I=ish-1,ieh
            do_i(I) = (OBC%OBC_mask_u(I,j) .and. &
                       (OBC%OBC_kind_u(I,j) == OBC_SIMPLE))
            if (do_i(I)) BT_cont%Fa_u_W0(I,j) = G%H_subroundoff*G%dy_u(I,j)
          enddo
...
subroutine zonal_flux_layer(u, h, hL, hR, uh, duhdu, visc_rem, dt, G, j, &
  ...
  real, dimension(NXMEMQ_),    intent(in)    :: u, visc_rem
  real, dimension(NXMEM_),     intent(in)    :: h, hL, hR
  real, dimension(NXMEMQ_),    intent(inout) :: uh, duhdu
  real,                        intent(in)    :: dt
  type(ocean_grid_type),       intent(inout) :: G
...
!                      areas when estimating the CFL number.
  ...
  do I=ish-1,ieh ; if (do_i(I)) then
    ...
    if (u(I) > 0.0) then
      if (vol_CFL) then ; CFL = (u(I) * dt) * (G%dy_u(I,j) * G%IDXDYh(i,j))
      else ; CFL = u(I) * dt * G%IDXh(i,j) ; endif
      curv_3 = hL(i) + hR(i) - 2.0*h(i)
      uh(I) = G%dy_u(I,j) * u(I) * &
          (hR(i) + CFL * (0.5*(hL(i) - hR(i)) + curv_3*(CFL - 1.5)))
      h_marg = hR(i) + CFL * ((hL(i) - hR(i)) + 3.0*curv_3*(CFL - 1.0))
    elseif (u(I) < 0.0) then
      if (vol_CFL) then ; CFL = (-u(I) * dt) * (G%dy_u(I,j) * G%IDXDYh(i+1,j))
      else ; CFL = -u(I) * dt * G%IDXh(i+1,j) ; endif
    ...
    duhdu(I) = G%dy_u(I,j) * h_marg * visc_rem(I)
  endif ; enddo
...
! real :: h_avg  ! The average thickness of a flux, in H.
  ...
  do k=1,nz ; do j=jsh,jeh ; do I=ish-1,ieh
    if (u(I,j,k) > 0.0) then
      if (vol_CFL) then ; CFL = (u(I,j,k) * dt) * (G%dy_u(I,j) * G%IDXDYh(i,j))
      else ; CFL = u(I,j,k) * dt * G%IDXh(i,j) ; endif
...
!     h_avg = hR(i,j,k) + CFL * (0.5*(hL(i,j,k) - hR(i,j,k)) + curv_3*(CFL - 1.5))
    ...
    elseif (u(I,j,k) < 0.0) then
      if (vol_CFL) then ; CFL = (-u(I,j,k)*dt) * (G%dy_u(I,j) * G%IDXDYh(i+1,j))
      else ; CFL = -u(I,j,k) * dt * G%IDXh(i+1,j) ; endif
...
subroutine zonal_flux_adjust(u, h_in, hL, hR, uhbt, uh_tot_0, duhdu_tot_0, &
  ...
  real, dimension(NXMEMQ_),            intent(out)   :: du
  real,                                intent(in)    :: dt
  type(ocean_grid_type),               intent(inout) :: G
  type(continuity_PPM_CS),             pointer       :: CS
  integer,                             intent(in)    :: j, ish, ieh
...
!  (out,opt) uh_3d - Volume flux through zonal faces = u*h*dy, H m2 s-1.
  ...
  logical :: full_prec, domore, do_i(SZIQ_(G))
  ...
  do itt=1,max_itts
    ...
    domore = .false.
    do I=ish-1,ieh ; if (do_i(I)) then
      if ((dt*min(G%IDXDYh(i,j),G%IDXDYh(i+1,j))*abs(uh_err(I)) > tol_eta) .or.&
...
!  (in)      do_I - A logical flag indiciating which I values to work on.
 ...
 ! Diagnose the zero-transport correction, du0.
  ...
  do I=ish-1,ieh
    if (do_i(I)) domore = .true.
    du_CFL(I) = (CFL_min * Idt) * G%DXu(I,j)
  ...
  do J=jsh-1,jeh
    ...
    if (present(vhbt) .or. do_aux .or. set_BT_cont) then
      ...
      do i=ish,ieh
        ...
        if (visc_rem_max(i) > 0.0) I_vrm = 1.0 / visc_rem_max(i)
        if (CS%vol_CFL) then
          dy_S = ratio_max(G%DXDYh(i,j), G%dx_v(i,J), 1000.0*G%DYh(i,j))
          dy_N = ratio_max(G%DXDYh(i,j+1), G%dx_v(i,J), 1000.0*G%DYh(i,j+1))
        else ; dy_S = G%DYh(i,j) ; dy_N = G%DYh(i,j+1) ; endif
        ...
        if (any_simple_OBC) then
          do i=ish,ieh
            do_i(i) = (OBC%OBC_mask_v(i,J) .and. &
                       (OBC%OBC_kind_v(i,J) == OBC_SIMPLE))
            if (do_i(i)) BT_cont%Fa_v_S0(i,J) = G%H_subroundoff*G%dx_v(I,j)
          enddo
...
subroutine merid_flux_layer(v, h, hL, hR, vh, dvhdv, visc_rem, dt, G, J, &
                            ish, ieh, do_i, vol_CFL)
  real, dimension(NXMEM_),        intent(in)    :: v, visc_rem
  real, dimension(NXMEM_,NYMEM_), intent(in)    :: h, hL, hR
  real, dimension(NXMEM_),        intent(inout) :: vh, dvhdv
  real,                           intent(in)    :: dt
  type(ocean_grid_type),          intent(inout) :: G
  integer,                        intent(in)    :: J, ish, ieh
...
!                       areas when estimating the CFL number.
  ...
  do i=ish,ieh ; if (do_i(i)) then
    if (v(i) > 0.0) then
      if (vol_CFL) then ; CFL = (v(i) * dt) * (G%dx_v(i,J) * G%IDXDYh(i,j))
      else ; CFL = v(i) * dt * G%IDYh(i,j) ; endif
      curv_3 = hL(i,j) + hR(i,j) - 2.0*h(i,j)
      vh(i) = G%dx_v(i,J) * v(i) * ( hR(i,j) + CFL * &
          (0.5*(hL(i,j) - hR(i,j)) + curv_3*(CFL - 1.5)) )
      h_marg = hR(i,j) + CFL * ((hL(i,j) - hR(i,j)) + &
                                  3.0*curv_3*(CFL - 1.0))
    elseif (v(i) < 0.0) then
      if (vol_CFL) then ; CFL = (-v(i) * dt) * (G%dx_v(i,J) * G%IDXDYh(i,j+1))
      else ; CFL = -v(i) * dt * G%IDYh(i,j+1) ; endif
      curv_3 = hL(i,j+1) + hR(i,j+1) - 2.0*h(i,j+1)
      vh(i) = G%dx_v(i,J) * v(i) * ( hL(i,j+1) + CFL * &
    ...
    dvhdv(i) = G%dx_v(i,J) * h_marg * visc_rem(i)
  endif ; enddo
...
! real :: h_avg  ! The average thickness of a flux, in H.
  ...
  do k=1,nz ; do J=jsh-1,jeh ; do i=ish,ieh
    if (v(i,J,k) > 0.0) then
      if (vol_CFL) then ; CFL = (v(i,J,k) * dt) * (G%dx_v(i,J) * G%IDXDYh(i,j))
      else ; CFL = v(i,J,k) * dt * G%IDYh(i,j) ; endif
      curv_3 = hL(i,j,k) + hR(i,j,k) - 2.0*h(i,j,k)
!     h_avg = hR(i,j,k) + CFL * (0.5*(hL(i,j,k) - hR(i,j,k)) + curv_3*(CFL - 1.5))
      h_marg = hR(i,j,k) + CFL * ((hL(i,j,k) - hR(i,j,k)) + &
                                3.0*curv_3*(CFL - 1.0))
    elseif (v(i,J,k) < 0.0) then
      if (vol_CFL) then ; CFL = (-v(i,J,k)*dt) * (G%dx_v(i,J) * G%IDXDYh(i,j+1))
      else ; CFL = -v(i,J,k) * dt * G%IDYh(i,j+1) ; endif
...
!  (out,opt) v_3d - Volume flux through meridional faces = v*h*dx, H m2 s-1.
  ...
  do itt=1,max_itts
    ...
    domore = .false.
    do i=ish,ieh ; if (do_i(i)) then
      if ((dt*min(G%IDXDYh(i,j),G%IDXDYh(i,j+1))*abs(vh_err(i)) > tol_eta) .or.&
...
!  (in)      do_i - A logical flag indiciating which i values to work on.
 ...
 ! Diagnose the zero-transport correction, dv0.
  ...
  do i=ish,ieh ; if (do_i(i)) then
    domore = .true.
    dv_CFL(i) = (CFL_min * Idt) * G%DYv(i,J)
...
subroutine PPM_reconstruction_x(h_in, h_l, h_r, G, LB, h_min, monotonic, simple_2nd)
  real, dimension(NXMEM_,NYMEM_), intent(in)  :: h_in
  real, dimension(NXMEM_,NYMEM_), intent(out) :: h_l, h_r
...
subroutine PPM_limit_pos(h_in, h_L, h_R, h_min, iis, iie, jis, jie)
  real, dimension(NXMEM_,NYMEM_), intent(in)     :: h_in
  real, dimension(NXMEM_,NYMEM_), intent(inout)  :: h_L, h_R
  real,                           intent(in)     :: h_min
