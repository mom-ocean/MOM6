...
!$OMP                          private(Idt_max2,H_eff_dx2,dyn_coef_max,ice_strength)
  ...
  if (CS%debug) then
    call uvchksum_pair("BT [uv]hbt", uhbt*GV%H_to_m, vhbt*GV%H_to_m, &
                       CS%debug_BT_HI,haloshift=0)
    call uvchksum_pair("BT Initial [uv]bt", ubt, vbt, &
                       CS%debug_BT_HI,haloshift=0)
    call hchksum(GV%H_to_kg_m2*eta, "BT Initial eta", CS%debug_BT_HI,haloshift=0)
    call uvchksum_pair("BT BT_force_[uv]", BT_force_u, BT_force_v, &
                       CS%debug_BT_HI,haloshift=0)
    if (interp_eta_PF) then
    ...
    call uvchksum_pair("BT Cor_ref_[uv]", Cor_ref_u, Cor_ref_v, &
                       CS%debug_BT_HI, haloshift=0)
    call uvchksum_pair("BT [uv]hbt0", uhbt0*GV%H_to_m, vhbt0*GV%H_to_m, &
                       CS%debug_BT_HI,haloshift=0)
    if (.not. use_BT_cont) then
      call uvchksum_pair("BT Dat[uv]", GV%H_to_m*Datu, GV%H_to_m*Datv, &
                         CS%debug_BT_HI, haloshift=1)
    endif
    call uvchksum_pair("BT wt_[uv]", wt_u, wt_v, G%HI, haloshift=1)
    call uvchksum_pair("BT frhat[uv]", CS%frhatu, CS%frhatv, G%HI, haloshift=1)
    call uvchksum_pair("BT bc_accel_[uv]", bc_accel_u, bc_accel_v, &
                       G%HI, haloshift=0)
    call uvchksum_pair("BT IDat[uv]", CS%IDatu, CS%IDatv, G%HI, haloshift=0)
    call uvchksum_pair("BT visc_rem_[uv]", visc_rem_u, visc_rem_v, &
                       G%HI,haloshift=1)
  endif
...
!GOMP                                  vhbt_sum,vbt_wtd_prev,vbt_wtd,BT_OBC)
    ...
    if (CS%debug_bt) then
      call uvchksum_pair("BT [uv]hbt just after OBC", uhbt*GV%H_to_m, &
...
!$OMP parallel do default(none) shared(isv,iev,jsv,jev,n,eta,eta_src,dtbt,CS,uhbt,vhbt,eta_wtd,wt_eta)
    ...
    if (CS%debug_bt) then
      ...
      call uvchksum_pair(trim(mesg)//" [uv]bt", ubt, vbt, &
...
!$OMP                          private(hatvtot,Ihatvtot,e_v,D_shallow_v,h_arith,h_harm,wt_arith)
  ...
  if (CS%debug) then
    call uvchksum_pair("btcalc frhat[uv]", CS%frhatu, CS%frhatv, G%HI, haloshift=1)
