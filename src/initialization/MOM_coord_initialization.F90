...
subroutine write_vertgrid_file(GV, US, param_file, directory)
  ...
  call write_field(trim(filepath), vars(1)%name, GV%Rlay, "append", scale=US%R_to_kg_m3)
  call write_field(trim(filepath), vars(2)%name, GV%g_prime, "append", scale=US%L_T_to_m_s**2*US%m_to_Z)
