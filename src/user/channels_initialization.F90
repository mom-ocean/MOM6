module channels_initialization
...
public channels_initialize_topography
...
! -----------------------------------------------------------------------------
!> This subroutine sets up the channels test case topography.
!> difference from channel: whereever it has vertical walls in channel setup,
!> it will be replaced by continental slopes, including in the sponge layer

subroutine channels_initialize_topography(D, G, param_file, max_depth)
  ...
  real :: reentrants, reentrantn  ! the non-dimensional latitudes of the southern and northern
                                  ! boundary of the reentrant channels
  real :: sdp = 4.0, ll=6.0    ! the width of the slope in Drake Passage, and the half width of continental slope

  character(len=40)  :: mod = "channels_initialize_topography" ! This subroutine's name.
  ...
  call MOM_mesg("  channels_initialization.F90, channels_initialize_topography: setting topography", 5)
  ...
  do i=is,ie                ! zonal grid points
    ...
    D(i,j) = 1.0 - spike(x-dx/2, ll/lonext)*spike(min(0.0, y-reentrantn-sdp/2.0), sdp) &                      ! Patagonia, west
                ...
                -spike(y-1.0, ll/latext) &                                                                 ! northern boundary
      ...
      if (D(i,j)<1.0 - sa .and. x>=ll/2/lonext .and. x<=1.0-ll/2/lonext .and. y>=ll/2/latext .and. y<=1.0-ll/2/latext) then ! make sure nowhere is deeper than sa
        D(i,j) = 1 - sa
...
end subroutine channels_initialize_topography
...
end module channels_initialization
