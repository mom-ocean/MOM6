module user_change_diffusivity_Hailu
...
type, public :: user_change_diff_CS ; private
  ...
  real :: rho_range(4)  !   4 values that define the coordinate potential
                        ...
                        ! Kd_add is added, in kg m-3; ascending
...
!! and a transition of diffusivity between this range and exterior
...
subroutine user_change_diff(h, tv, G, CS, Kd, Kd_int, T_f, S_f, Kd_int_add)
  ...
  if (.not.use_EOS) return ! But what if EOS is not used?
                           ! e.g. buoyancy/density is used, not salinity or temperature
  store_Kd_add = .false.
    ...
    if (present(Kd_int)) then
      do K=2,nz ; do i=is,ie
        ...
        if (rho_fn * lat_fn > 0.0) then
          ! Kd_int is not initialized locally!
...
end module user_change_diffusivity_Hailu
