...
use cvmix_kappa_shear, only : cvmix_kappa_shear_column , cvmix_kappa_shear_CS, cvmix_kappa_shear_init
...
subroutine kappa_shear_column(kappa, tke, dt, nzc, f2, surface_pres, &
  ...
  if(CS%use_cvmix_kappa_shear_column) then
      call cvmix_kappa_shear_column(GV%ke, nzc, dt, f2, use_temperature, &
                                     dz, dbuoy_dT, dbuoy_dS, &
                                     u, v, T, Sal, &
                                     kappa, tke, kappa_avg, tke_avg, CS%cvmix_CS)

  else !The following block should bitwise produce the answers of the above cvmix call. 
...
! ----------------------------------------------------
  ...
  endif !if(use_cvmix_kappa_shear_column)
...
!    id_clock_setup = cpu_clock_id('Ocean KS setup',grain=CLOCK_ROUTINE)
  ...
  call get_param(param_file, mdl, "KAPPA_SHEAR_USE_CVMIX",CS%use_cvmix_kappa_shear_column, &
                 "If true, use the Jackson et.al. kappa-shear columm physics \n"//&
                 "that is exported to CVMIX package.", default=.true.)

  if(CS%use_cvmix_kappa_shear_column) then
      call cvmix_kappa_shear_init(CS%RiNo_crit , CS%Shearmix_rate, CS%max_RiNo_it, CS%kappa_0, &
                                  CS%FRi_curvature, CS%C_N, CS%C_S, CS%lambda, CS%lambda2_N_S, &
                                  CS%kappa_tol_err, CS%TKE_bg, CS%eliminate_massless, CS%max_KS_it, &
                                  CS%Prandtl_turb,CS%nkml, CS%cvmix_CS )
  endif
