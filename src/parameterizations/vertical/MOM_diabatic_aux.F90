...
subroutine diagnoseMLDbyDensityDifference(id_MLD, h, tv, densityDiff, G, GV, US, diagPtr, &
  ...
  do j=js,je
    ...
    call calculate_density(tv%T(:,j,1), tv%S(:,j,1), pRef_MLD, rhoSurf, G%HI, tv%eqn_of_state)
    do i=is,ie
      deltaRhoAtK(i) = 0.
      MLD(i,j) = 0.
      if (id_N2>0) then
    ...
    do k=2,nz
      ...
      call calculate_density(tv%T(:,j,k), tv%S(:,j,k), pRef_MLD, deltaRhoAtK, G%HI, tv%eqn_of_state)
      do i = is, ie
    ...
    if (id_N2>0) then  ! Now actually calculate stratification, N2, below the mixed layer.
      ...
      !    N2_region_set(i) = .true.
      ! endif
      call calculate_density(T_subML, S_subML, pRef_N2, rho_subML, G%HI, tv%eqn_of_state)
      call calculate_density(T_deeper, S_deeper, pRef_N2, rho_deeper, G%HI, tv%eqn_of_state)
    ...
    if (calculate_energetics) then
      ...
      do k=1,nz
        ...
        call calculate_specific_vol_derivs(T2d(:,k), tv%S(:,j,k), p_lay(:), &
                 dSV_dT(:,j,k), dSV_dS(:,j,k), G%HI, tv%eqn_of_state)
