...
! the wind stress is applied as a stress boundary condition.
    ...
    if (PRESENT(taux_bot)) then
      do I=Isq,Ieq
        taux_bot(I,j) = US%L_T2_to_m_s2*Rho0 * (u(I,j,nz)*CS%a_u(I,j,nz+1))
      enddo
      if (CS%Channel_drag) then ; do k=1,nz ; do I=Isq,Ieq
        taux_bot(I,j) = taux_bot(I,j) + US%L_T2_to_m_s2*Rho0 * (Ray(I,k)*u(I,j,k))
      enddo ; enddo ; endif
    endif
...
! the wind stress is applied as a stress boundary condition.
    ...
    if (present(tauy_bot)) then
      do i=is,ie
        tauy_bot(i,J) = Rho0 * (US%L_T2_to_m_s2*v(i,J,nz)*CS%a_v(i,J,nz+1))
      enddo
      if (CS%Channel_drag) then ; do k=1,nz ; do i=is,ie
        tauy_bot(i,J) = tauy_bot(i,J) + Rho0 * (US%L_T2_to_m_s2*Ray(i,k)*v(i,J,k))
      enddo ; enddo ; endif
    endif
