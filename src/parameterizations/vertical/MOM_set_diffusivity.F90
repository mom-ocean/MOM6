...
subroutine set_diffusivity(u, v, h, tv, fluxes, visc, dt, G, CS, &
  ...
  real, dimension(NXMEM_,NYMEM_,NKMEM_),  intent(out)   :: Kd
  real, dimension(NXMEM_,NYMEM_,NK_INTERFACE_), optional, intent(out) :: Kd_int
...
! another explicit parameterization of Kd.
  ...
  do j=js,je
    ...
    if (CS%double_diffusion) then
      call double_diffusion(tv, h, T_f, S_f, j, G, CS, KT_extra, KS_extra)
  ...
  ! Add the input turbulent diffusivity.
    ...
    call find_TKE_to_Kd(h, tv, dRho_int, N2_lay, j, dt, G, CS, TKE_to_Kd, &
                        maxTKE, kb)
    ...
    if (CS%ML_radiation) &
      call add_MLrad_diffusivity(h, fluxes, j, G, CS, Kd, TKE_to_Kd, Kd_int)

    ! Add the Nikurashin and / or tidal bottom-driven mixing
    if (CS%Int_tide_dissipation .or. CS%Lee_wave_dissipation) &
      call add_int_tide_diffusivity(h, N2_bot, j, TKE_to_Kd, maxTKE, G, CS, &
                                    dd, N2_lay, Kd, Kd_int)
...
subroutine find_TKE_to_Kd(h, tv, dRho_int, N2_lay, j, dt, G, CS, TKE_to_Kd, maxTKE, kb)
  real, dimension(NXMEM_,NYMEM_,NKMEM_), intent(in)   :: h
  type(thermo_var_ptrs),                intent(in)    :: tv
  real, dimension(NXMEM_,NK_INTERFACE_), intent(in)   :: dRho_int
  real, dimension(NXMEM_,NKMEM_),       intent(in)    :: N2_lay
  integer,                              intent(in)    :: j
  real,                                 intent(in)    :: dt
  type(ocean_grid_type),                intent(in)    :: G
  type(set_diffusivity_CS),             pointer       :: CS
  real, dimension(NXMEM_,NKMEM_),       intent(out)   :: TKE_to_Kd, maxTKE
  integer, dimension(NXMEM_),           intent(out)   :: kb
  ...
  if (CS%bulkmixedlayer) then
    ...
    do i=is,ie
      ...
      do k=kmb+1,nz-1 ; if (Rcv_kmb(i) <= G%Rlay(k)) exit ; enddo
      kb(i) = k
    ...
    call set_density_ratios(h, tv, kb, G, CS, j, ds_dsp1, rho_0)
  else ! not bulkmixedlayer
    kb_min = 2 ; kmb = 0
    do i=is,ie ; kb(i) = 1 ; enddo
    call set_density_ratios(h, tv, kb, G, CS, j, ds_dsp1)
  endif
...
subroutine find_N2(h, tv, T_f, S_f, fluxes, j, G, CS, dRho_int, N2_lay, N2_int, N2_bot)
  real, dimension(NXMEM_,NYMEM_,NKMEM_), intent(in)   :: h
  type(thermo_var_ptrs),                 intent(in)   :: tv
  real, dimension(NXMEM_,NYMEM_,NKMEM_), intent(in)   :: T_f, S_f
  type(forcing),                         intent(in)   :: fluxes
  integer,                               intent(in)   :: j
  type(ocean_grid_type),                 intent(in)   :: G
  type(set_diffusivity_CS),              pointer      :: CS
  real, dimension(NXMEM_,NK_INTERFACE_), intent(out)  :: dRho_int, N2_int
  real, dimension(NXMEM_,NKMEM_),        intent(out)  :: N2_lay
  real, dimension(NXMEM_),               intent(out)  :: N2_bot
...
subroutine double_diffusion(tv, h, T_f, S_f, j, G, CS, Kd_T_dd, Kd_S_dd)
  ...
  type(ocean_grid_type),                 intent(in)  :: G
  type(set_diffusivity_CS),              pointer     :: CS
  real, dimension(NXMEM_,NK_INTERFACE_), intent(out) :: Kd_T_dd, Kd_S_dd
...
subroutine add_drag_diffusivity(h, u, v, tv, fluxes, visc, j, &
  ...
  integer,                                intent(in)    :: j
  real, dimension(NXMEM_,NKMEM_),         intent(in)    :: TKE_to_Kd, maxTKE
  integer, dimension(NXMEM_),             intent(in)    :: kb
  type(ocean_grid_type),                  intent(in)    :: G
  type(set_diffusivity_CS),               pointer       :: CS
  real, dimension(NXMEM_,NYMEM_,NKMEM_),  intent(inout) :: Kd
...
! by the bottom drag.
  ...
  do K=2,nz ; Rint(K) = 0.5*(G%Rlay(k-1)+G%Rlay(k)) ; enddo
  ...
  do i=is,ie
    ...
    Rho_top(i) = G%Rlay(1)
    if (CS%bulkmixedlayer .and. do_i(i)) Rho_top(i) = G%Rlay(kb(i)-1)
  enddo

  do k=nz-1,2,-1 ; domore = .false.
    do i=is,ie ; if (do_i(i)) then
      htot(i) = htot(i) + G%H_to_m*h(i,j,k)
      rho_htot(i) = rho_htot(i) + G%Rlay(k)*(G%H_to_m*h(i,j,k))
      if (htot(i)*G%Rlay(k-1) <= (rho_htot(i) - gh_sum_top(i))) then
...
subroutine add_MLrad_diffusivity(h, fluxes, j, G, CS, Kd, TKE_to_Kd, Kd_int)
  real, dimension(NXMEM_,NYMEM_,NKMEM_), intent(in)    :: h
  type(forcing),                         intent(in)    :: fluxes
  integer,                               intent(in)    :: j
  type(ocean_grid_type),                 intent(in)    :: G
  type(set_diffusivity_CS),              pointer       :: CS
  real, dimension(NXMEM_,NYMEM_,NKMEM_), intent(inout) :: Kd
  real, dimension(NXMEM_,NKMEM_),        intent(in)    :: TKE_to_Kd
  real, dimension(NXMEM_,NYMEM_,NK_INTERFACE_), optional, intent(inout) :: Kd_int
...
subroutine add_int_tide_diffusivity(h, N2_bot, j, TKE_to_Kd, max_TKE, G, CS, &
                                    dd, N2_lay, Kd, Kd_int )
  real, dimension(NXMEM_,NYMEM_,NKMEM_), intent(in)    :: h
  real, dimension(NXMEM_),               intent(in)    :: N2_bot
  real, dimension(NXMEM_,NKMEM_),        intent(in)    :: N2_lay
  integer,                               intent(in)    :: j
  real, dimension(NXMEM_,NKMEM_),        intent(in)    :: TKE_to_Kd, max_TKE
  type(ocean_grid_type),                 intent(in)    :: G
  type(set_diffusivity_CS),              pointer       :: CS
  ...
  do i=is,ie ; htot(i) = 0.0 ; Inv_int(i) = 0.0 ; Inv_int_lee(i) = 0.0 ; enddo
  do k=1,nz ; do i=is,ie
    htot(i) = htot(i) + G%H_to_m*h(i,j,k)
  enddo ; enddo
  ...
  use_Polzin = ((CS%Int_tide_dissipation .and. (CS%int_tide_profile == POLZIN_09)) .or. &
                (CS%lee_wave_dissipation .and. (CS%lee_wave_profile == POLZIN_09)))
  use_Simmons = ((CS%Int_tide_dissipation .and. (CS%int_tide_profile == STLAURENT_02)) .or. &
                 (CS%lee_wave_dissipation .and. (CS%lee_wave_profile == STLAURENT_02)))
...
! Both Polzin and Simmons
  do i=is,ie
    ...
    if (associated(dd%Fl_itidal)) dd%Fl_itidal(i,j,nz) = TKE_itidal_rem(i)
  enddo
...
! Simmons
  if ( use_Simmons ) then
    do k=nz-1,2,-1 ; do i=is,ie
      ...
      if (TKE_itide_lay + TKE_Niku_lay > max_TKE(i,k)) then
         frac_used = max_TKE(i,k) / (TKE_itide_lay + TKE_Niku_lay)
      ...
      Kd_add  = TKE_to_Kd(i,k) * (TKE_itide_lay + TKE_Niku_lay)
...
subroutine set_density_ratios(h, tv, kb, G, CS, j, ds_dsp1, rho_0)
  ...
  type(set_diffusivity_CS),              pointer       :: CS
  integer,                               intent(in)    :: j
  real, dimension(NXMEM_,NKMEM_),        intent(out)   :: ds_dsp1
  real, dimension(NXMEM_,NKMEM_), optional, intent(in) :: rho_0
...
! interfaces above and below the buffer layer and the next denser layer.
        ...
        do k3=1,kmb
          a(k3+1) = (G%Rlay(k) - Rcv(i,k3)) * I_Drho
        enddo
...
subroutine set_diffusivity_init(Time, G, param_file, diag, CS, diag_to_Z_CSp)
