...
public absorbRemainingSW, absorbRemainingSw2exp, sumSWoverBands
...
type, public :: optics_type
  ...
  !BGR/ Added Two_Exp_Form if using 2-exponential form decay
  logical :: Two_Exp_Form ! .true. if two exponential form

  !BGR/ Added sw_1st_exp_ratio for use with 2-exponential decay shortwave
  !       scheme (i.e. Jerlov type).
  real :: sw_1st_exp_ratio   !   The ratio for the 1st exponential decay
                             ! if using a 2-e decay scheme.  The 2nd is assumed
                             ! equal to 1 minus this value.
  ...
  real, pointer, dimension(:,:,:,:) :: &
    opacity_band_2nd => NULL()  ! SW optical depth per unit thickness (1/m)
                                ! Number of radiation bands is most rapidly varying (first) index.
...
subroutine sumSWoverBands(G, GV, h, opacity_band, opacity_band_2nd,nsw, j, dt, &
                          H_limit_fluxes, absorbAllSW, iPen_SW_bnd, netPen,&
                          TwoExpForm,First_Exp_Ratio)
...
! buoyancy fluxes for use in KPP. This routine does not update the state.
  ...
  real, dimension(SZI_(G),SZK_(G)),      intent(in)    :: h
  real, dimension(:,:,:),                intent(in)    :: opacity_band
  real, dimension(:,:,:),                intent(in)    :: opacity_band_2nd
  integer,                               intent(in)    :: nsw
  integer,                               intent(in)    :: j
  real,                                  intent(in)    :: dt
  real,                                  intent(in)    :: H_limit_fluxes
  ...
  logical,                               intent(in) :: TwoExpForm
  Real,                                  intent(in) :: First_Exp_Ratio
! Arguments:
    ...
    do i=is,ie
      ...
      if (h(i,k) > 0.0) then
        do n=1,nsw ; if (Pen_SW_bnd(n,i) > 0.0) then
          ...
          !{
          if (TwoExpForm) then
            opt_depth_2nd = h(i,k)*GV%H_to_m * opacity_band_2nd(n,i,k)
...
!> Apply shortwave heating below surface boundary layer.
subroutine absorbRemainingSW2exp(G, GV, h, opacity_band, opacity_band_2nd, nsw, j, dt,    &
                             H_limit_fluxes, &
                             adjustAbsorptionProfile, absorbAllSW, T, Pen_SW_bnd, &
                             TwoExpForm, First_Exp_Ratio, &
                             eps, ksort, htot, Ttot, TKE, dSV_dT)
...
!  (inout,opt) TKE      = the TKE sink from mixing the heating throughout a layer, in J m-2.
  ...
  real :: opt_depth         ! optical depth of a layer (non-dim)
  real :: opt_depth_2nd     ! 2nd comp. of optical depth of a layer in 2-exp form
  real :: exp_OD            ! sw_1st_exp_ratio * exp(-opt_depth) (non-dim)
  real :: exp_OD_2nd        ! (1.-sw_1st_exp_ratio) * exp(opt_depth_2nd)
  real :: heat_bnd          ! heating due to absorption in the current
                            ! layer by the current band, including any piece that
                            ! is moved upward (K H units)
  real :: SWa               ! fraction of the absorbed shortwave that is
...
end subroutine absorbRemainingSW2exp
