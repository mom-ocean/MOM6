...
!                        The default is .true.
  ...
  do j=js,je
    ...
    do i=is,ie ; if (htot(i) > 0.0) then
      ...
      T_ml(i) = Ttot(i) * Ih ; S_ml(i) = Stot(i) * Ih
...
! these unused layers (but not currently in the code).
    ...
    if (CS%ML_resort) then
      call cpu_clock_begin(id_clock_resort)
      call resort_ML(h, eps, d_ea, d_eb, R0, Rcv, T, S, ksort, G, CS, &
...
! treated in conjuction.
    ...
    if (CS%nkbl == 1) then
      call mixedlayer_detrain_1(h, R0, htot, R0_ml, dt, dt__diag, d_ea, d_eb, &
               j, G, CS, T, S, Rcv, T_ml, S_ml, Rcv_ml, dRcv_dT, dRcv_dS, max_BL_det)
...
subroutine resort_ML(h, eps, d_ea, d_eb, R0, Rcv, T, S, ksort, G, CS, &
  ...
  real, dimension(NIMEM_,NKMEM_),    intent(inout) :: h, eps, d_ea, d_eb
...
subroutine mixedlayer_detrain_2(h, R0, h_ml, R0_ml, dt, dt_diag, d_ea, j, G, &
                                CS, T, S, Rcv, T_ml, S_ml, Rcv_ml, &
                                dRcv_dT, dRcv_dS, dR0_dT, dR0_dS, max_BL_det)
  real, dimension(NIMEM_,NKMEM_), intent(inout) :: h, R0
  real, dimension(NIMEM_),        intent(inout) :: h_ml
  real, dimension(NIMEM_),        intent(in)    :: R0_ml
  ...
  real, dimension(NIMEM_),        intent(in)    :: dRcv_dT, dRcv_dS, dR0_dT, dR0_dS
...
subroutine mixedlayer_detrain_1(h, R0, h_ml, R0_ml, dt, dt_diag, d_ea, d_eb, &
  ...
  real, dimension(NIMEM_,NKMEM_), intent(inout) :: h, R0
  real, dimension(NIMEM_),        intent(inout) :: h_ml, R0_ml
  real,                           intent(in)    :: dt, dt_diag
...
! Set default, read and log parameters
 ...
 !CS%ML_presort_nz_conv_adj = 0 ! Not needed AJA?
  ...
  CS%id_ML_depth = register_diag_field('ocean_model', 'h_ML', G%axesT1, &
      Time, 'Surface mixed layer depth', 'meter')
  CS%id_TKE_wind = register_diag_field('ocean_model', 'TKE_wind', G%axesT1, &
      Time, 'Wind-stirring source of mixed layer TKE', 'meter3 second-3')
  CS%id_TKE_RiBulk = register_diag_field('ocean_model', 'TKE_RiBulk', G%axesT1, &
      Time, 'Mean kinetic energy source of mixed layer TKE', 'meter3 second-3')
  CS%id_TKE_conv = register_diag_field('ocean_model', 'TKE_conv', G%axesT1, &
      Time, 'Convective source of mixed layer TKE', 'meter3 second-3')
  CS%id_TKE_pen_SW = register_diag_field('ocean_model', 'TKE_pen_SW', G%axesT1, &
      Time, 'TKE consumed by mixing penetrative shortwave radation through the mixed layer', 'meter3 second-3')
  CS%id_TKE_mixing = register_diag_field('ocean_model', 'TKE_mixing', G%axesT1, &
      Time, 'TKE consumed by mixing that deepens the mixed layer', 'meter3 second-3')
  CS%id_TKE_mech_decay = register_diag_field('ocean_model', 'TKE_mech_decay', G%axesT1, &
      Time, 'Mechanical energy decay sink of mixed layer TKE', 'meter3 second-3')
  CS%id_TKE_conv_decay = register_diag_field('ocean_model', 'TKE_conv_decay', G%axesT1, &
      Time, 'Convective energy decay sink of mixed layer TKE', 'meter3 second-3')
  CS%id_TKE_conv_s2 = register_diag_field('ocean_model', 'TKE_conv_s2', G%axesT1, &
      Time, 'Spurious source of mixed layer TKE from sigma2', 'meter3 second-3')
  CS%id_PE_detrain = register_diag_field('ocean_model', 'PE_detrain', G%axesT1, &
      Time, 'Spurious source of potential energy from mixed layer detrainment', 'Watt meter-2')
  CS%id_PE_detrain2 = register_diag_field('ocean_model', 'PE_detrain2', G%axesT1, &
      Time, 'Spurious source of potential energy from mixed layer only detrainment', 'Watt meter-2')
  CS%id_h_mismatch = register_diag_field('ocean_model', 'h_miss_ML', G%axesT1, &
      Time, 'Summed absolute mismatch in entrainment terms', 'meter')
  CS%id_Hsfc_used = register_diag_field('ocean_model', 'Hs_used', G%axesT1, &
      Time, 'Surface region thickness that is used', 'meter')
  CS%id_Hsfc_max = register_diag_field('ocean_model', 'Hs_max', G%axesT1, &
      Time, 'Maximum surface region thickness', 'meter')
  CS%id_Hsfc_min = register_diag_field('ocean_model', 'Hs_min', G%axesT1, &
