...
!$OMP end parallel
  ...
  if (CS%debug) then
    call uvchksum_pair(Kh_u(:,:,:),"Kh_u", Kh_v(:,:,:),"Kh_v",G%HI,haloshift=0)
    call uvchksum_pair(int_slope_u(:,:,:),"int_slope_u", &
                       int_slope_v(:,:,:),"int_slope_v",G%HI,haloshift=0)
    ...
    if (use_stored_slopes) then
      call uvchksum_pair(VarMix%slope_x(:,:,:),"VarMix%slope_x", &
                         VarMix%slope_y(:,:,:),"VarMix%slope_y",G%HI,haloshift=0)
    endif
...
!$OMP parallel do default(none) shared(is,ie,js,je,MEKE,VarMix)
  ...
  if (CS%debug) then
    call uvchksum_pair(uhD(:,:,:)*H_to_m,"thickness_diffuse uhD", &
                       vhD(:,:,:)*H_to_m,"thickness_diffuse vhD",G%HI,haloshift=0)
    call uvchksum_pair(uhtr(:,:,:)*H_to_m,"thickness_diffuse uhtr", &
                       vhtr(:,:,:)*H_to_m,"thickness_diffuse vhtr",G%HI,haloshift=0)
