...
type, public :: oda_incupd_CS ; private
  ...
  type(p3d) :: Ref_val(MAX_FIELDS_) !< The values to which the fields are updated.
  type(p3d) :: Ref_oda_u  !< The values to which the u-velocities are updated.
  type(p3d) :: Ref_oda_v  !< The values to which the v-velocities are updated.
  ...
  type(p3d) :: Ref_h  !< Grid on which reference data is provided (older code).
  type(p3d) :: Ref_p  !< grid of layer pressure increment
  type(p3d) :: Ref_hu !< u-point grid on which reference data is provided (older code).
  type(p3d) :: Ref_hv !< v-point grid on which reference data is provided (older code).
...
subroutine initialize_oda_incupd( G, GV, param_file, CS, data_p, nz_data)
  ...
  real, dimension(SZI_(G),SZJ_(G),nz_data), intent(in) :: data_p !< The int. depth incr. 
                                                                 !! [H ~> m or kg m-2].
...
! Set default, read and log parameters
  ...
  if (nhours_incupd == 0) then 
     ...
     CS%nstep_incupd = nhours_incupd*3600./dt_baclin
  endif
  ...
  CS%nz_data = nz_data
  allocate(CS%Ref_p%p(G%isd:G%ied,G%jsd:G%jed,CS%nz_data))
  CS%Ref_p%p(:,:,:) = 0.0 ;
  do j=G%jsc,G%jec; do i=G%isc,G%iec ; do k=1,CS%nz_data
      CS%Ref_p%p(i,j,k) = data_p(i,j,k)
  enddo;  enddo ; enddo
...
subroutine set_up_oda_incupd_field(sp_val, G, GV, f_ptr, CS)
  ...
  allocate(CS%Ref_val(CS%fldno)%p(G%isd:G%ied,G%jsd:G%jed,CS%nz_data))
  CS%Ref_val(CS%fldno)%p(:,:,:) = 0.0
  do j=G%jsc,G%jec ; do i=G%isc,G%iec
    do k=1,CS%nz_data
      CS%Ref_val(CS%fldno)%p(i,j,k) = sp_val(i,j,k)
    enddo
  enddo ; enddo
...
subroutine set_up_oda_incupd_vel_field(u_val, v_val, G, GV, u_ptr, v_ptr, CS)
  ...
  ! stores the reference profile
  allocate(CS%Ref_oda_u%p(G%isdB:G%iedB,G%jsd:G%jed,CS%nz_data))
  CS%Ref_oda_u%p(:,:,:) = 0.0
  do j=G%jsc,G%jec ; do i=G%iscB,G%iecB
    do k=1,CS%nz_data
      CS%Ref_oda_u%p(i,j,k) = u_val(i,j,k)
    enddo
  enddo ; enddo
  CS%var_u%p => u_ptr

  allocate(CS%Ref_oda_v%p(G%isd:G%ied,G%jsdB:G%jedB,CS%nz_data))
  CS%Ref_oda_v%p(:,:,:) = 0.0
  do j=G%jscB,G%jecB ; do i=G%isc,G%iec
    do k=1,CS%nz_data
      CS%Ref_oda_v%p(i,j,k) = v_val(i,j,k)
    enddo
  enddo ; enddo
  CS%var_v%p => v_ptr
...
subroutine apply_oda_incupd(h, dt, G, GV, US, CS)
  ...
  ! get interface depth from h
  allocate(tmp_p(G%isd:G%ied,G%jsd:G%jed,nz+1)) ; tmp_p(:,:,:) = 0.0 
  allocate(tmp_h(G%isd:G%ied,G%jsd:G%jed,nz  )) ; tmp_h(:,:,:) = 0.0
  do j=js,je ; do i=is,ie
    tmp_p(i,j,nz+1) = G%bathyT(i,j)
  enddo ; enddo
  do k=nz,1,-1 ; do j=js,je ; do i=is,ie
    tmp_p(i,j,k) = tmp_p(i,j,k+1) - h(i,j,k)
  enddo ; enddo ; enddo

  ! add increments to interface depths
  do k=1,nz  ; do j=js,je ; do i=is,ie
    tmp_p(i,j,k) = tmp_p(i,j,k) + q*CS%Ref_p%p(i,j,k)
  enddo ; enddo ; enddo
  
  ! get the new layer thickness 
  do k=1,nz  ; do j=js,je ; do i=is,ie
    tmp_h(i,j,k) = max(0.0, tmp_p(i,j,k+1) - tmp_p(i,j,k))
  enddo ; enddo ; enddo
  ...
  do m=1,CS%fldno
    ...
    do j=js,je ; do i=is,ie
      tmp_val2(1:nz_data) = CS%Ref_val(m)%p(i,j,1:nz_data)
      do k=1,nz
        h_col(k)=h(i,j,k)
      enddo
      call remapping_core_h(CS%remap_cs, nz_data, tmp_h(i,j,1:nz_data), tmp_val2, &
             nz, h_col, tmp_val1, h_neglect, h_neglect_edge)
!      tmp_val1(1:nz)=tmp_val2(1:nz)
...
! add increments to u
  ...
  do j=js,je ; do i=isB,ieB
    tmp_val2(1:nz_data) = 0.5*(CS%Ref_oda_u%p(i  ,j,1:nz_data) + &
                               CS%Ref_oda_u%p(i+1,j,1:nz_data))
    do k=1,nz
      hu(k)    = 0.5 * (tmp_h(i,j,k) + tmp_h(i+1,j,k))
      h_col(k) = 0.5 * (    h(i,j,k) +     h(i+1,j,k))
    enddo
    call remapping_core_h(CS%remap_cs, nz_data, hu(1:nz_data), tmp_val2, &
                 nz, h_col, tmp_val1, h_neglect, h_neglect_edge)
!    tmp_val1(1:nz)=tmp_val2(1:nz)
...
! add increments to v
  ...
  do j=jsB,jeB ; do i=is,ie
    tmp_val2(1:nz_data) = 0.5*(CS%Ref_oda_v%p(i,j  ,1:nz_data) + &
                               CS%Ref_oda_v%p(i,j+1,1:nz_data))
    do k=1,nz
      hv(k)    = 0.5 * (tmp_h(i,j,k) + tmp_h(i,j+1,k))
      h_col(k) = 0.5 * (    h(i,j,k) +     h(i,j+1,k))
    enddo
    call remapping_core_h(CS%remap_cs, nz_data, hv(1:nz_data), tmp_val2, &
                 nz, h_col, tmp_val1, h_neglect, h_neglect_edge)
!    tmp_val1(1:nz)=tmp_val2(1:nz)
