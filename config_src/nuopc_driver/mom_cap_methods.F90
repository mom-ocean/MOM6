module mom_cap_methods
  ...
  use ESMF,                only: ESMF_TimeInterval, ESMF_TimeIntervalTeg
    ...
    do j = jsc, jec
      ...
      do i = isc, iec
        ...
        if (ocn_public%frazil(ig,jg) > 0.0) then
           ...
           dataPtr_Fioo_q(i1,j1) = ocn_public%frazil(i,j) * grid%mask2dT(ig,jg) * I_time_int
        else
           ! Melt_potential: change from J/m^2 to W/m^2
           dataPtr_Fioo_q(i1,j1) = -ocn_public%melt_potential(i,j) * grid%mask2dT(ig,jg) * I_time_int !* ncouple_per_day
