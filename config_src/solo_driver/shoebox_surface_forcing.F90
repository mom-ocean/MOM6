!> Wind and buoyancy forcing for the shoebox configurations
module shoebox_surface_forcing
...
public shoebox_wind_forcing
public shoebox_buoyancy_forcing
public shoebox_surface_forcing_init

!> This control structure should be used to store any run-time variables
!! associated with the shoebox forcing.  It can be readily modified
...
type, public :: shoebox_surface_forcing_CS ; private
  ...
  logical :: first_call = .true. !< True until shoebox_buoyancy_forcing has been called
end type shoebox_surface_forcing_CS

contains

!> Sets the surface wind stresses, forces%taux and forces%tauy for the
!! shoebox forcing configuration.
subroutine shoebox_wind_forcing(sfc_state, forces, day, G, CS)
  type(surface),                 intent(inout) :: sfc_state !< A structure containing fields that
                                                    !! describe the surface state of the ocean.
  type(mech_forcing),            intent(inout) :: forces !< A structure with the driving mechanical forces
  type(time_type),               intent(in)    :: day !< Time used for determining the fluxes.
  type(ocean_grid_type),         intent(inout) :: G !< Grid structure.
  type(shoebox_surface_forcing_CS), pointer  :: CS !< Control structure for this module.
  ! Local variable
...
end subroutine shoebox_wind_forcing


!> Surface fluxes of buoyancy for the shoebox configurations.
subroutine shoebox_buoyancy_forcing(sfc_state, fluxes, day, dt, G, CS)
  type(surface),                 intent(inout) :: sfc_state !< A structure containing fields that
                                                    !! describe the surface state of the ocean.
  ...
  real :: den, rho1 = 1027.459, rho2 = 1023.303, rho3 = 1027.093 ! south, equator, north
  integer :: i, j, is, ie, js, je
  ...
  if (CS%use_temperature) then
    call MOM_error(FATAL, "shoebox_buoyancy_forcing: " // &
...
! set up target surface density profile
  ...
  if (CS%restorebuoy) then
    if (CS%use_temperature) then      ! if temperature is used to restore buoyancy
      call MOM_error(FATAL, "shoebox_buoyancy_surface_forcing: " // &
...
end subroutine shoebox_buoyancy_forcing
...
!> Initializes the shoebox control structure.
subroutine shoebox_surface_forcing_init(Time, G, param_file, diag, CS)
  ...
  type(shoebox_surface_forcing_CS), pointer :: CS !< A pointer that is set to point to the control structure
                                                    !! for this module
  ! This include declares and sets the variable "version".
#include "version_variable.h"
  ! Local variables
  character(len=40)  :: mod = "shoebox_surface_forcing" ! This module's name.

  if (associated(CS)) then
    call MOM_error(WARNING, "shoebox_surface_forcing_init called with an associated "// &
                             "control structure.")
    return
  endif
  allocate(CS)
...
end subroutine shoebox_surface_forcing_init
...
end module shoebox_surface_forcing
