!> Wind and buoyancy forcing for the shoebox3 configurations
module shoebox3_surface_forcing
...
public shoebox3_buoyancy_forcing
public shoebox3_wind_forcing
public shoebox3_surface_forcing_init

!> This control structure should be used to store any run-time variables
!! associated with the shoebox3 forcing.  It can be readily modified
...
type, public :: shoebox3_surface_forcing_CS ; private
  ...
  logical :: first_call = .true. !< True until shoebox3_buoyancy_forcing has been called
end type shoebox3_surface_forcing_CS

contains


subroutine shoebox3_buoyancy_forcing(sfc_state, fluxes, day, dt, G, CS)
  ...
  type(shoebox3_surface_forcing_CS), pointer  :: CS !< Control structure for this module.
  ! Local variables
  ...
  if (CS%use_temperature) then
    call MOM_error(FATAL, "shoebox3_buoyancy_forcing: " // &
...
! set up target surface density profile
  ...
  if (CS%restorebuoy) then
    if (CS%use_temperature) then      ! if temperature is used to restore buoyancy
      call MOM_error(FATAL, "shoebox3_buoyancy_surface_forcing: " // &
...
end subroutine shoebox3_buoyancy_forcing

!> Sets the surface wind stresses, forces%taux and forces%tauy for the
!! shoebox3 forcing configuration.
subroutine shoebox3_wind_forcing(sfc_state, forces, day, G, CS)
  type(surface),                 intent(inout) :: sfc_state !< A structure containing fields that
...
end subroutine shoebox3_wind_forcing
...
!> Initializes the shoebox3 control structure.
subroutine shoebox3_surface_forcing_init(Time, G, param_file, diag, CS)
  ...
  type(shoebox3_surface_forcing_CS), pointer :: CS !< A pointer that is set to point to the control structure
                                                    !! for this module
  ! This include declares and sets the variable "version".
#include "version_variable.h"
  ! Local variables
  character(len=40)  :: mod = "shoebox3_surface_forcing" ! This module's name.

  if (associated(CS)) then
    call MOM_error(WARNING, "shoebox3_surface_forcing_init called with an associated "// &
                             "control structure.")
    return
  endif
  allocate(CS)
...
end subroutine shoebox3_surface_forcing_init
...
end module shoebox3_surface_forcing
