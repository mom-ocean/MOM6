!> Wind and buoyancy forcing for the channel 4 configurations
...
!> surface wind peak is 0.14/2

module channel5_surface_forcing
...
public channel5_wind_forcing
public channel5_buoyancy_forcing
public channel5_surface_forcing_init


!> This control structure should be used to store any run-time variables
!! associated with the channel5 forcing.  It can be readily modified
...
type, public :: channel5_surface_forcing_CS ; private
  ...
  logical :: first_call = .true. !< True until channel5_buoyancy_forcing has been called
end type channel5_surface_forcing_CS
...
!! channel5 forcing configuration.
subroutine channel5_wind_forcing(sfc_state, forces, day, G, CS)
  type(surface),                 intent(inout) :: sfc_state !< A structure containing fields that
                                                    !! describe the surface state of the ocean.
  type(mech_forcing),            intent(inout) :: forces !< A structure with the driving mechanical forces
  type(time_type),               intent(in)    :: day !< Time used for determining the fluxes.
  type(ocean_grid_type),         intent(inout) :: G !< Grid structure.
  type(channel5_surface_forcing_CS), pointer  :: CS !< Control structure for this module.
  ! Local variable
...
end subroutine channel5_wind_forcing


!> Surface fluxes of buoyancy for the channel5 configurations.
subroutine channel5_buoyancy_forcing(sfc_state, fluxes, day, dt, G, CS)
  type(surface),                 intent(inout) :: sfc_state !< A structure containing fields that
                                                    !! describe the surface state of the ocean.
  ...
  real :: den, rhos = 1037.5, rhon = 1034.162     ! sothern and northern boundary densities of the channel5
  integer :: i, j, is, ie, js, je
  ...
  if (CS%use_temperature) then
    call MOM_error(FATAL, "channel5_buoyancy_forcing: " // &
  ...
  if (CS%restorebuoy) then
    if (CS%use_temperature) then      ! if temperature is used to restore buoyancy
      call MOM_error(FATAL, "channel5_buoyancy_surface_forcing: " // &
...
end subroutine channel5_buoyancy_forcing
...
!> Initializes the channel5 control structure.
subroutine channel5_surface_forcing_init(Time, G, param_file, diag, CS)
  ...
  type(channel5_surface_forcing_CS), pointer :: CS !< A pointer that is set to point to the control structure
                                                    !! for this module
  ! This include declares and sets the variable "version".
#include "version_variable.h"
  ! Local variables
  character(len=40)  :: mod = "channel5_surface_forcing" ! This module's name.

  if (associated(CS)) then
    call MOM_error(WARNING, "channel5_surface_forcing_init called with an associated "// &
                             "control structure.")
    return
  endif
  allocate(CS)
...
end subroutine channel5_surface_forcing_init
...
end module channel5_surface_forcing
