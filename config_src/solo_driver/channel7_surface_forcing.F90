!> Wind and buoyancy forcing for the channel 7 configurations
!> same wind configuration as channel 4, where peak wind stress in the SO = 0.2 Pa
!> different buoyancy config: in the range of sponge (32S-30S), density = rho(32S)
!> otherwise rho profile remains the same as channel 4

module channel7_surface_forcing
...
public channel7_wind_forcing
public channel7_buoyancy_forcing
public channel7_surface_forcing_init


!> This control structure should be used to store any run-time variables
!! associated with the channel7 forcing.  It can be readily modified
...
type, public :: channel7_surface_forcing_CS ; private
  ...
  logical :: first_call = .true. !< True until channel7_buoyancy_forcing has been called
end type channel7_surface_forcing_CS
...
!! channel7 forcing configuration.
subroutine channel7_wind_forcing(sfc_state, forces, day, G, CS)
  type(surface),                 intent(inout) :: sfc_state !< A structure containing fields that
                                                    !! describe the surface state of the ocean.
  type(mech_forcing),            intent(inout) :: forces !< A structure with the driving mechanical forces
  type(time_type),               intent(in)    :: day !< Time used for determining the fluxes.
  type(ocean_grid_type),         intent(inout) :: G !< Grid structure.
  type(channel7_surface_forcing_CS), pointer  :: CS !< Control structure for this module.
  ! Local variable
...
end subroutine channel7_wind_forcing


!> Surface fluxes of buoyancy for the channel7 configurations.
subroutine channel7_buoyancy_forcing(sfc_state, fluxes, day, dt, G, CS)
  type(surface),                 intent(inout) :: sfc_state !< A structure containing fields that
                                                    !! describe the surface state of the ocean.
  ...
  real :: den, rhos = 1037.5, rhon = 1034.785, spongelen=2.0     
  ! sothern and northern boundary densities of the channel7; latitudinal width of sponge
  integer :: i, j, is, ie, js, je
  integer :: isd, ied, jsd, jed

  is = G%isc ; ie = G%iec ; js = G%jsc ; je = G%jec
  isd = G%isd ; ied = G%ied ; jsd = G%jsd ; jed = G%jed
  latext = G%len_lat; spongelen = spongelen/latext

  ! Allocate and zero out the forcing arrays, as necessary.  This portion is
  ! usually not changed.
  if (CS%use_temperature) then
    call MOM_error(FATAL, "channel7_buoyancy_forcing: " // &
  ...
  if (CS%restorebuoy .and. CS%first_call) then
    ...
    do j = js, je
      ...
      if (y<1-spongelen) then                         ! if the point is outside of the sponge
        CS%buoy_restore(is,j) = (rhon-rhos)*y+rhos    ! linearly decreased density with latitudes
      else                                            ! otherwise rho is just rho(32S)
        CS%buoy_restore(is,j) = (rhon-rhos)*(1-spongelen)+rhos
      endif
    enddo
  ...
  if (CS%restorebuoy) then
    if (CS%use_temperature) then      ! if temperature is used to restore buoyancy
      call MOM_error(FATAL, "channel7_buoyancy_surface_forcing: " // &
...
end subroutine channel7_buoyancy_forcing
...
!> Initializes the channel7 control structure.
subroutine channel7_surface_forcing_init(Time, G, param_file, diag, CS)
  ...
  type(channel7_surface_forcing_CS), pointer :: CS !< A pointer that is set to point to the control structure
                                                    !! for this module
  ! This include declares and sets the variable "version".
#include "version_variable.h"
  ! Local variables
  character(len=40)  :: mod = "channel7_surface_forcing" ! This module's name.

  if (associated(CS)) then
    call MOM_error(WARNING, "channel7_surface_forcing_init called with an associated "// &
                             "control structure.")
    return
  endif
  allocate(CS)
...
end subroutine channel7_surface_forcing_init
...
end module channel7_surface_forcing
