!> Wind and buoyancy forcing for the channel configurations
module channel_surface_forcing
...
public channel_wind_forcing
public channel_buoyancy_forcing
public channel_surface_forcing_init

!> This control structure should be used to store any run-time variables
!! associated with the channel forcing.  It can be readily modified
...
type, public :: channel_surface_forcing_CS ; private
  ...
  logical :: first_call = .true. !< True until channel_buoyancy_forcing has been called
end type channel_surface_forcing_CS

contains

!> Sets the surface wind stresses, forces%taux and forces%tauy for the
!! channel forcing configuration.
subroutine channel_wind_forcing(sfc_state, forces, day, G, CS)
  type(surface),                 intent(inout) :: sfc_state !< A structure containing fields that
                                                    !! describe the surface state of the ocean.
  type(mech_forcing),            intent(inout) :: forces !< A structure with the driving mechanical forces
  type(time_type),               intent(in)    :: day !< Time used for determining the fluxes.
  type(ocean_grid_type),         intent(inout) :: G !< Grid structure.
  type(channel_surface_forcing_CS), pointer  :: CS !< Control structure for this module.
  ! Local variable
...
end subroutine channel_wind_forcing


!> Surface fluxes of buoyancy for the channel configurations.
subroutine channel_buoyancy_forcing(sfc_state, fluxes, day, dt, G, CS)
  type(surface),                 intent(inout) :: sfc_state !< A structure containing fields that
                                                    !! describe the surface state of the ocean.
  ...
  real :: den, rhos = 1027.459, rhon = 1025.335     ! sothern and northern boundary densities of the channel
  integer :: i, j, is, ie, js, je
  ...
  if (CS%use_temperature) then
    call MOM_error(FATAL, "channel_buoyancy_forcing: " // &
  ...
  if (CS%restorebuoy) then
    if (CS%use_temperature) then      ! if temperature is used to restore buoyancy
      call MOM_error(FATAL, "channel_buoyancy_surface_forcing: " // &
...
end subroutine channel_buoyancy_forcing
...
!> Initializes the channel control structure.
subroutine channel_surface_forcing_init(Time, G, param_file, diag, CS)
  ...
  type(channel_surface_forcing_CS), pointer :: CS !< A pointer that is set to point to the control structure
                                                    !! for this module
  ! This include declares and sets the variable "version".
#include "version_variable.h"
  ! Local variables
  character(len=40)  :: mod = "channel_surface_forcing" ! This module's name.

  if (associated(CS)) then
    call MOM_error(WARNING, "channel_surface_forcing_init called with an associated "// &
                             "control structure.")
    return
  endif
  allocate(CS)
...
end subroutine channel_surface_forcing_init
...
end module channel_surface_forcing
